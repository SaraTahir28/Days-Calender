...
export function getEventDate(event, year, month) { //event from days.json, year as per user selection, month in which event occurred
  ...
  const targetDayName = event.dayName; //e.g targetDayName = "Tuesday" and occurrence index becomes 1.
  const occurrenceIndex = occurrenceMap[event.occurrence];//e.g second tuesday of the month.

  const daysInThisMonth = dayjs().year(year).month(month).daysInMonth(); //daysInThisMonth tells us how many days to loop over. 
  const matchingDays = []; //[] of exact dates of desired weekday

  //compare all days of the month with event's dayname
  for (let d = 1; d <= daysInThisMonth; d++) {
    const date = dayjs().year(year).month(month).date(d); // Day.js object that represents a specific calendar date for every day
    const dayName = date.format("dddd"); // gives day of the week for the date- string.
